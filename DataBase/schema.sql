-- =============================================================================
-- OGEFREM Tickets — Schéma complet (état au 2026-06-22)
-- Moteur : MySQL / MariaDB (XAMPP) — utf8mb4
--
-- Installation neuve :
...
CREATE TABLE roles (
  ...
  hierarchy_level TINYINT UNSIGNED NOT NULL COMMENT '0=admin,1=directeur,2=sous-directeur,3=chef,4=agent/chef bureau'
) ENGINE=InnoDB;
...
CREATE TABLE category_service_routing (
...
CREATE TABLE tickets (
  ...
  routed_service_id TINYINT UNSIGNED NULL,
  ...
  CONSTRAINT fk_tickets_routed_service FOREIGN KEY (routed_service_id) REFERENCES dantic_services(id),
  ...
  UNIQUE KEY uk_tickets_tracking_token (tracking_token)
) ENGINE=InnoDB;
...
CREATE INDEX idx_tickets_routed_service ON tickets(routed_service_id);
...
CREATE TABLE ticket_co_interventions (
  id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  ticket_id INT UNSIGNED NOT NULL,
...
CREATE INDEX idx_tci_agent_status ON ticket_co_interventions(agent_id, status);

CREATE TABLE ticket_consignes (
  id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
  ticket_id INT UNSIGNED NOT NULL,
...
CREATE INDEX idx_consignes_ticket ON ticket_consignes(ticket_id, created_at);
...
CREATE TABLE rapports_valides (
  ...
  UNIQUE KEY uk_rv_report_id (report_id)
) ENGINE=InnoDB;

CREATE INDEX idx_rv_validated_at ON rapports_valides(validated_at);

-- ---------------------------------------------------------------------------
-- Notifications, sessions, rate limiting
-- ---------------------------------------------------------------------------
...
CREATE TABLE monthly_bundle_comments (
  id INT UNSIGNED PRIMARY KEY AUTO_INCREMENT,
...
CREATE INDEX idx_mbc_bundle ON monthly_bundle_comments(bundle_id, created_at);
